...
(
    SELECT
        ...
        -- arrayLast returns the last element of arrayEnumerate(src_arr) for
        -- which the predicate is true.  Since arrayEnumerate yields [1..N],
        -- the returned value IS the 1-based index we want.  Default 0 when
        -- no element matches (→ fallback below to the actual last index).
        arrayLast(
