...
в DataLens нет deep-link вида «создать воркбук и сразу импортировать
в него JSON» — endpoint `/workbooks/import` обязательно требует
существующий `workbookId`, поэтому первый шаг руками.
...
3. Внутри коллекции нажми **Создать воркбук** → дай имя, например
   `Дашборд атрибуции`. Откроется пустой воркбук.
4. В пустом воркбуке меню **⋯** (справа вверху) → **Импортировать**.
   Выбери файл `datalens/dashboard.json` из репо.
5. На шаге привязки подключения: DataLens покажет пустое
...
6. Подтверди импорт. Все три датасета автоматически привяжутся к
