...
- **source_transitions** — отдельная таблица с матрицей переходов
  prev→next (используется для pivot-матрицы «матрица переходов»
...
DataLens со всей логикой: подключение к ClickHouse, **5 датасетов**
(`ds_attribution` на view `v_attribution_results`, `ds_transitions`
на `v_source_transitions`, `ds_assisted` на `v_assisted_long`, плюс
`ds_touchpoints` и `ds_chains` на SQL-подзапросах), **9 чартов** (3
KPI + 100%-стек + time-series + grouped-bar + ассоциированные +
матрица переходов + сводная таблица), три глобальных селектора
(Период / Модель / Источник) с прописанными Связями.

**Что ты получаешь после импорта:**

- Индикаторы за период: атрибутированный доход, конверсии, среднее число
  касаний до конверсии.
- Сравнение 5 моделей (First / Last / **Last Significant** / Linear /
  Time Decay) в стек-баре и группированных столбцах — видно, как разные
  модели по-разному кредитуют источники, в т.ч. как `last_significant`
  переносит часть кредита с Direct/Internal на реальные каналы.
- Time-series топ-источников для выбранной модели — переключи модель,
  график перестроится.
- **Прямые vs ассоциированные конверсии** по top-10 источников
  (на отдельном датасете `ds_assisted` в long-формате).
- **Матрица переходов «откуда → куда»** — pivot-таблица top-100
  переходов с цветовым кодированием (заменяет Sankey).
